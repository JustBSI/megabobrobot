...
database_cursor = database.cursor()  # Обьект курсора для управления БД.
...
def change_health(message):
    """
    Функция, принимающяя информацию о здоровье от пользователя.
...
def change_note_text(message):
    ...
    keyboard = telebot.types.ReplyKeyboardMarkup(resize_keyboard=True)
    keyboard.row('Назад', 'Меню')
    if message.text.isdigit():  # Проверка что пользователь ввёл ID заметки, а не ерунду.
        note_id = message.text  # Сохранение введённого ID для передачи в другую функцию.
        ...
        action(message, "Введи ID, а не текст!")
        bot.register_next_step_handler(message, change_note_text)
...
def change_note_in_db(message, note_id):
    ...
    keyboard.row('Назад', 'Меню')
    if message.text != 'Назад' and message.text != 'Меню':
...
def action(message, text="Выбери нужный пункт!"):
    """
    Функция определения действий для бота в зависимости от нажатой пользователем кнопки.

    :param message: сообщение от пользователя
    :param text: текст предупреждения пользователю
    """
